...
- **Repository automation**. Currently, it only covers issues and epic grooming.
  ...
  The 'Lock Threads' action and the `alex-page/github-project-automation-plus@v0.8.3` action both require a GitHub token to interact with the GitHub API. The token is fetched from the GitHub secrets store and is passed to the actions in their respective workflow files.
